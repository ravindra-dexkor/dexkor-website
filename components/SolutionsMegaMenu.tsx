...
const industries = [
  { name: "SaaS", icon: Layers, active: true, color: "text-indigo-600", bgColor: "bg-indigo-50" },
  { name: "Fintech", icon: Building2, color: "text-slate-600", bgColor: "bg-slate-50" },
  { name: "Logistics", icon: Truck, color: "text-slate-600", bgColor: "bg-slate-50" },
  { name: "EdTech", icon: GraduationCap, color: "text-slate-600", bgColor: "bg-slate-50" },
  { name: "D2C / E-commerce", icon: ShoppingBag, color: "text-slate-600", bgColor: "bg-slate-50" },
  { name: "Healthcare", icon: Heart, color: "text-slate-600", bgColor: "bg-slate-50" },
];
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="lg:hidden px-6 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
             ...
             <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Solutions</span>
          </div>

          <div className="max-w-7xl mx-auto px-6 py-6 grid grid-cols-1 md:grid-cols-12 gap-x-8 gap-y-6">
            {/* BROWSE BY USE CASE */}
            <div className="col-span-1 md:col-span-4">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Browse by Use Case</h3>
                ...
                {useCases.map((useCase) => (
                  <div 
                    ...
                    className={cn(
                        ...
                        useCase.active ? "bg-slate-50" : "hover:bg-slate-50/50"
                    )}
                  >
                    {useCase.active && (
                    ...
                    <div className={cn(
                        ...
                        useCase.active ? "bg-white text-indigo-600 shadow-sm" : "bg-slate-100 text-slate-400 group-hover:text-indigo-600 group-hover:bg-indigo-50"
                    )}>
                      <useCase.icon className="w-5 h-5" />
                    </div>
                    <div>
                      <div className="flex items-center justify-between">
                        <span className={cn("font-bold text-sm transition-colors", useCase.active ? "text-indigo-600" : "text-slate-700 group-hover:text-indigo-600")}>
                            {useCase.title}
                        </span>
                      ...
                      <p className="text-[11px] text-slate-500 mt-0.5 leading-tight">{useCase.description}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="col-span-1 md:col-span-8 flex flex-col">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Explore by Industry</h3>
                ...
                {industries.map((industry) => (
                  <div 
                    ...
                    className={cn(
                        ...
                        industry.active ? "border-indigo-100 bg-indigo-50/50 shadow-sm" : "border-slate-100 hover:border-indigo-100 hover:bg-slate-50"
                    )}
                  >
                    <div className={cn(
                        "w-10 h-10 rounded-lg flex items-center justify-center transition-colors",
                        industry.active ? "bg-white text-indigo-600 shadow-sm" : "bg-slate-50 text-slate-600"
                    )}>
                        <industry.icon className="w-5 h-5" />
                    </div>
                    <span className={cn("text-[11px] font-bold text-center", industry.active ? "text-indigo-600" : "text-slate-600")}>
                        {industry.name}
                    </span>
                  </div>
                ))}
              ...
              {/* FEATURED CONTENT */}
              <div className="bg-slate-50/80 rounded-2xl p-6 border border-slate-100 relative overflow-hidden flex-1 group cursor-pointer">
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                    <div className="space-y-4">
                        <span className="text-[10px] font-bold bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full uppercase">For SaaS Teams</span>
                        ...
                        <div className="grid grid-cols-3 gap-4 py-2 border-y border-slate-200/50">
                            {[
                    ...
                    <div className="hidden lg:block relative">
                         <div className="bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden group-hover:scale-[1.02] transition-transform duration-500">
                             <div className="p-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                                ...
                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Customer Health Overview</span>
                             </div>
                             <div className="p-3">
                                <img 
             ...
             <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      ...
                      <p className="text-[11px] text-slate-500 max-w-sm">Tell us about your goals and we'll recommend the right solution for your team.</p>
                   </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-6 md:gap-10">
                   ...
                   <div className="flex items-center gap-4">
                      ...
                      <p className="text-[10px] font-medium text-slate-500 leading-tight">
                         Trusted by <span className="font-bold text-slate-900">500+</span> teams<br />
                         across <span className="font-bold text-slate-900">20+</span> countries
                      </p>
                   </div>
                </div>
             </div>
